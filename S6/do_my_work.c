...
int doMyWork(void* long_computing(void*), void* backup_computing(void*), char* string, int ret_value)
{
    ...
    pthread_t thread_2; 
    void* value;
    if (pthread_create(&thread, NULL, long_computing, string) != 0) return -1;
    if (pthread_join(thread, &value) != 0) return -2;
    if( *(int*)value != ret_value)
    {
        if (pthread_create(&thread_2, NULL, backup_computing, string) != 0) return -3;
        if (pthread_join(thread_2, &value) != 0) return -4;
    }
    return *(int*)value;
}